...
public class SegmentStatusManager {
  ...
  /**
   * This method reads the load metadata file with tablestatus lock
   *
   * @param identifier
   * @return
   */
  public static LoadMetadataDetails[] readLoadMetadataWithLock(AbsoluteTableIdentifier identifier) {
    String metadataFileName = CarbonTablePath.getTableStatusFilePath(identifier.getTablePath());
    SegmentStatusManager segmentStatusManager = new SegmentStatusManager(identifier);
    ICarbonLock carbonLock = segmentStatusManager.getTableStatusLock();
    boolean lockStatus = carbonLock.lockWithRetries();
    try {
      if (lockStatus) {
        LOG.info("Acquired lock for" + metadataFileName);
        return readTableStatusFile(metadataFileName);
      } else {
        throw new IOException("Unable to unlock Table lock for" + metadataFileName);
      }
    } catch (IOException e) {
      LOG.error("Fail to get tablestatus" + e.getMessage());
      return new LoadMetadataDetails[0];
    } finally {
      if (lockStatus) {
        if (carbonLock.unlock()) {
          LOG.info("Table unlocked successfully for table" + metadataFileName);
        } else {
          LOG.error("Unable to unlock Table lock for table" + metadataFileName);
        }
      }
    }
  }
  ...
  public static LoadMetadataDetails[] readTableStatusFile(String tableStatusPath)
    ...
    LoadMetadataDetails[] loadMetadataDetails = new LoadMetadataDetails[0];

    // When storing table status file in object store, reading of table status file may
    // fail (receive EOFException or JsonSyntaxException)
    // when table status file is being modifying
    // so here we retry multiple times before throwing EOFException or JsonSyntaxException
    while (retry > 0) {
      try {
        ...
        try {
          ...
          TimeUnit.MILLISECONDS.sleep(READ_TABLE_STATUS_RETRY_TIMEOUT);
  ...
  public static void deleteLoadsAndUpdateMetadata(CarbonTable carbonTable, boolean isForceDeletion,
    ...
    LoadMetadataDetails[] metadataDetails =
        SegmentStatusManager.readLoadMetadataWithLock(carbonTable.getAbsoluteTableIdentifier());
